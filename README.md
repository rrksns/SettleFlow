...
- **App 레벨**: Consumer 로직에서 DuplicateKeyException을 핸들링하여, 중복 메시지 수신 시 에러가 아닌 WARN 로그를 남기고 정상 처리(Ack) 하도록 구현. 무한 재시도(Retry Storm) 방지
...
| 항목 | 정보 |
|------|------|
| Name | Seol (8년 차 Backend Developer) |
| Contact | (이메일/블로그 주소 등) |
